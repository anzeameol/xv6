...
void sub_process(int p_left[2], int i)
{
    ...
    */
    ...
    // prime = get a number from left neighbor
    close(p_left[1]);
    if ((num_read = read(p_left[0], &prime, 1)) < 0)
    {
        fprintf(2, "sub_process %d: read failed\n", i);
        exit(1);
    }
    if (num_read == 0)
    {
    ...
    p_right[0] = p_right[1] = -1;
    // End
    printf("prime %d\n", prime);
    while (1)
    {
        // m = get a number from left neighbor
        if ((num_read = read(p_left[0], &m, 1)) < 0)
        {
            fprintf(2, "sub_process %d: read failed\n", i);
            exit(1);
        }
        ...
        if (p_right[0] < 0)
        {
            if (pipe(p_right) < 0)
            {
                fprintf(2, "sub_process %d: pipe failed\n", i);
                exit(1);
            }
            pid = fork();
            if (pid == 0)
...
void composites()
{
    ...
    */
    ...
    if (pipe(p_right) < 0)
    {
        fprintf(2, "composities: pipe failed\n");
        exit(1);
    }
    pid = fork();
    if (pid == 0)
